...
class CheckoutService {
  ...
  Future<void> updateAddressData(
    ...
    Map addData = addressData.docs[0].data();
    // print("37");
    // print(addData);
    // print("39");
    // print(addressData);
    ...
    // List savedAddress = addressData.docs[0][0].data()['address'];
    String documentId = addressData.docs[0].id;
    List savedAddress = addData['address'];
  ...
  Future<void> newShippingAddress(Map address) async {
    ...
    for (DocumentSnapshot docRef in data.docs) {
      print(docRef);
    }
  ...
  Future<void> placeNewOrder(Map orderDetails) async {
    ...
    List productItems = items.docs.map((doc) {
      return doc['products'];
    }).toList()[0];
  ...
  Future<List> listPlacedOrder() async {
    List orderList = [];
    String uid = await _userService.getUserId();
    QuerySnapshot orders = await _orderReference
        // .orderBy('placedDate', descending: true)
        .where('userId', isEqualTo: uid)
        .get();
    for (DocumentSnapshot order in orders.docs) {
      Map orderMap = new Map();
      Map subOrderMap = order.data();
      // orderMap['orderDate'] = order[0].data()['placedDate'];
      orderMap['orderDate'] = subOrderMap['placedDate'];
      List orderData = [];
      for (int i = 0; i < subOrderMap['items'].length; i++) {
        Map tempOrderData = new Map();
        tempOrderData['quantity'] = subOrderMap['items'][i]['quantity'];
        DocumentSnapshot docRef =
            await _productReference.doc(subOrderMap['items'][i]['id']).get();
        Map subDocRef = docRef.data();
        print(subDocRef);
        tempOrderData['productImage'] = subDocRef['imageId'][0];
        tempOrderData['productName'] = subDocRef['name'];
        tempOrderData['price'] = subDocRef['price'];
