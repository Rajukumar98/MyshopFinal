...
class ProfileService {
  ...
  Future<Map> getUserProfile() async {
    ...
    QuerySnapshot profileData = await _firestore
        ...
        .get();
    Map proData = profileData.docs[0].data();

    print(proData);
    profileDetails['fullName'] = proData['fullName'];
    profileDetails['mobileNumber'] = proData['mobileNumber'];
  ...
  Future<void> updateAccountDetails(
    ...
    return _firestore.collection('users').doc(documentId).set(
        {'fullName': fullName, 'mobileNumber': mobileNumber, 'userId': uid});
  }
